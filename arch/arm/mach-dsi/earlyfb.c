...
//
// arch/arm/mach-dsi/earlyfb.c — Nintendo DSi sub-screen early framebuffer.
//
...
//
// Public API (declared in <asm/mach/dsi-earlyfb.h>):
//   void dsi_early_fb_init(void)   — called once from head-dsi.S
//   void dsi_early_putc(int c)     — called per-char from debug/dsi.S
...
/* ---- DSi sub-screen MMIO ------------------------------------------------ */
#define DSI_VRAMCNT_C   0x04000242u /* VRAM bank C routing register (8-bit) */
#define DSI_DISPCNT_SUB 0x04001000u /* sub-engine display control (32-bit)  */
#define DSI_BG2CNT_SUB  0x0400100Cu /* sub-engine BG2 control (16-bit)      */
...
#define DSI_BG2PA_SUB 0x04001020u /* horizontal pixel increment, 8.8 fp   */
#define DSI_BG2PB_SUB 0x04001022u /* horizontal line increment,  8.8 fp   */
#define DSI_BG2PC_SUB 0x04001024u /* vertical pixel increment,   8.8 fp   */
#define DSI_BG2PD_SUB 0x04001026u /* vertical line increment,    8.8 fp   */
#define DSI_BG2X_SUB  0x04001028u /* reference point X, 20.8 fp, 32-bit   */
#define DSI_BG2Y_SUB  0x0400102Cu /* reference point Y, 20.8 fp, 32-bit   */
#define DSI_BGPAL_SUB 0x05000400u /* sub-engine BG palette base (16-bit)   */
#define DSI_VRAM_C    0x06200000u /* VRAM bank C mapped to sub BG          */
...
/* BG2CNT: 8bpp extended-rotation bitmap, 256×256 */
...
static int dsi_col;
static int dsi_row;
static bool dsi_fb_ready;

/* ---- 8×8 bitmap font for printable ASCII 0x20..0x7F -------------------- */
/*
 * Each entry is 8 bytes: one byte per pixel row, MSB = leftmost pixel.
 * Source: IBM VGA 8×8 character ROM (public domain).
 */
...
/* ---- Raw MMIO helpers --------------------------------------------------- */
/* No ioremap at this stage; no-MMU means physical == virtual. */
static inline void io_wr8(u32 addr, u8 v) { *(volatile u8 *)addr = v; }
static inline void io_wr16(u32 addr, u16 v) { *(volatile u16 *)addr = v; }
static inline void io_wr32(u32 addr, u32 v) { *(volatile u32 *)addr = v; }

/* ---- Framebuffer scroll ------------------------------------------------- */
static void fb_scroll_up(void) {
    volatile u32 *fb = (volatile u32 *)DSI_VRAM_C;
    ...
    for (i = 0; i < (ROWS - 1) * words_per_row; i++) fb[i] = fb[i + words_per_row];
    for (; i < total_words; i++) fb[i] = 0;
}

/* ---- Glyph rasterizer --------------------------------------------------- */
static void putglyph(int col, int row, int c) {
    volatile u8 *fb = (volatile u8 *)DSI_VRAM_C;
    ...
    for (r = 0; r < FONT_H; r++) {
        u8 bits = glyph[r];
        for (p = 0; p < FONT_W; p++) {
            fb[(row * FONT_H + r) * FB_W + col * FONT_W + p] = (bits & (0x80u >> p)) ? 1 : 0;
        }
    }
}
...
void dsi_early_fb_init(void) {
    ...
    io_wr8(DSI_VRAMCNT_C, VRAMCNT_C_SUB_BG);
    io_wr32(DSI_DISPCNT_SUB, DISPCNT_VAL);
    io_wr16(DSI_BG2CNT_SUB, BG2CNT_BMP8_256x256);

    /* Identity affine matrix: 1:1 pixel mapping, no rotation/scaling. */
    io_wr16(DSI_BG2PA_SUB, 0x0100);
    io_wr16(DSI_BG2PB_SUB, 0x0000);
    io_wr16(DSI_BG2PC_SUB, 0x0000);
    io_wr16(DSI_BG2PD_SUB, 0x0100);
    io_wr32(DSI_BG2X_SUB, 0x00000000);
    io_wr32(DSI_BG2Y_SUB, 0x00000000);

    /* Palette: index 0 = black (background), index 1 = white (text). */
    io_wr16(DSI_BGPAL_SUB, 0x0000);
    io_wr16(DSI_BGPAL_SUB + 2, PAL_WHITE);

    /* Clear framebuffer to index 0 (black). */
    for (i = 0; i < FB_W * FB_H; i++) fb[i] = 0;
